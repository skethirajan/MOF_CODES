...
class System:
    ...
    @staticmethod
    def _compare_two_atoms(atom1: ase.atoms.Atom, atom2: ase.atoms.Atom) -> bool:
        if atom1.index == atom2.index and atom1.symbol == atom2.symbol and \
                np.array_equal(atom1.position, atom2.position):
            return True
        else:
            return False
    ...
    def __init__(self, atoms: Union[ase.atoms.Atoms, list[ase.atoms.Atom], str],
        ...
        self._is_ase_atoms: bool = True

        # becomes dict[int, int] only when self.is_ase_atoms is false
        self._map_indices: Optional[dict[int, int]] = None
        ...
        if isinstance(atoms, str):  # atoms is a file
            ...
            if all(isinstance(i, ase.atoms.Atom) for i in atoms):  # all elements in atoms list is ase.atoms.Atom type
                if kwargs.get("cell") and isinstance(kwargs["cell"], ase.cell.Cell):
                    self._is_ase_atoms = False
                    self.atoms = self._convert_to_ase_atoms(atoms, kwargs["cell"])
                    self._map_indices = self._mapping_indices(atoms)
    ...
    @property
    def _natural_cutoffs(self) -> list[float]:
    ...
    @property
    def _neighborlist(self) -> ase.neighborlist.NeighborList:
        ...
        neighborlist = NeighborList(cutoffs=self._natural_cutoffs, self_interaction=False, bothways=True)
        neighborlist.update(self.atoms)
        return neighborlist

    @property
    def _connected_components(self) -> tuple[int, np.ndarray]:
        nl = self._neighborlist
        return connected_components(nl.get_connectivity_matrix())

    @property
    def _clusters(self) -> dict[int, list[ase.atoms.Atom]]:
        n_components, component_list = self._connected_components
        clusters = {}
        for cluster_id in range(0, n_components):
            indices = np.where(component_list == cluster_id)[0]  # gives indices w.r.t. ase.atoms.Atoms
            if self._is_ase_atoms:
                cluster_atoms = [self.atoms[index] for index in indices]
            else:  # self.list_of_atom[index] below will return ase.atoms.Atom with correct index
                cluster_atoms = [self.list_of_atom[index] for index in indices]
            cluster_atoms.sort(key=lambda atom: atom.symbol)
            clusters.update({cluster_id+1: cluster_atoms})
        return clusters
    ...
    def _get_index_connectivity(self, atom: ase.atoms.Atom) -> list[int]:
        ...
        if self._is_ase_atoms:
            ...
            return [self._map_indices[ind] for ind in neighbor_indices]  # return indices w.r.t list[ase.atoms.Atom]
    ...
    def _get_indices_connectivity(self) -> dict[int, list[int]]:
        ...
        for atom in self.atoms:
            neighbor_indices = self._get_index_connectivity(atom)  # these neighbor indices are always correct
            if self._is_ase_atoms:
                indices_connectivity.update({atom.index: neighbor_indices})
            else:
                indices_connectivity.update({self._map_indices[atom.index]: neighbor_indices})
